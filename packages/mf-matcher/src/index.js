...
module.exports= function mfFilter(entry, options = {}) {
    ...
    if (entry.unsaturation !== undefined) {
        if (entry.unsaturation<minUnsaturation || entry.unsaturation > maxUnsaturation) return;
        if (onlyIntegerUnsaturation && ! Number.isInteger(entry.unsaturation)) return;
        if (onlyNonIntegerUnsaturation && Number.isInteger(entry.unsaturation)) return;
    }
