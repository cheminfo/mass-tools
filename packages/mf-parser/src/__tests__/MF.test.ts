...
test('C1-2', () => {
  const mf = new MF('C1-2', { flattenLimit: 2 });
  const parts = mf.toParts();
...
test('(CH2)1+ useless parenthesis', () => {
...
test('Types getInfo default shape', () => {
  const mf = new MF('C10#1H20');
  const info = mf.getInfo();
...
test('Types getInfo custom shape', () => {
  const mf = new MF('C10#1H20');
