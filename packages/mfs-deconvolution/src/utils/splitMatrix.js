...
export function splitMatrix(matrix) {
  ...
  for (let row = 0; row < matrix.length; row++) {
    for (let column of nonZeroColumns[row]) {
      ...
      if (rowsMapping[row] !== 65535) {
        continue
      }
      if (columnsMapping[column] !== 65535) {
        continue
      }
      currentMappingID++;
  ...
  return { maxMappingID: currentMappingID, matrices }

}

function appendSubMatrix(matrices, matrix) {
  for (const entry of matrices) {
    const submatrix = new Array(entry.rows.length).fill(0).map(() => new Float64Array(entry.columns.length))
  ...
  while ((nextRow = rowsTodo.pop()) !== undefined) {
    ...
    for (let column of nonZeroColumns[nextRow]) {
      ...
      for (let internalRow of nonZeroRows[column]) {
        if (rowsMapping[internalRow] === 65535) {
